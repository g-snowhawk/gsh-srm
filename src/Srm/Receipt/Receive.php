...
class Receive extends Response
{
    ...
    public function updateBillingDate()
    {
        ...
        $filter = $this->app->cnf('application:srm_check_holidays') ?? [];
        if ($post['force_update'] ?? 'no' === 'yes') {
            $filter = [];
        }
        if (!empty($post['billing_date']) && in_array(7, $filter)) { // Check holiday
            $is_holiday = false;
            $billing_day = date('w', strtotime($post['billing_date']));
            $billing_ymd = date('Ymd', strtotime($post['billing_date']));

            $url = $this->app->cnf('application:srm_check_holidays_calendar');
            if (!empty($url) && in_array(7, $filter)) {
                ...
                if (preg_match_all('/BEGIN:VEVENT[\s]*(.+?)[\s]*END:VEVENT/s', $output, $matches)) {
                    foreach ($matches[1] as $match) {
                        ...
                        preg_match('/DTEND;VALUE=DATE:(.+)/', $match, $hit);
                        $end = date('Y', strtotime($hit[1]));

                        foreach (range($start, $end) as $y) {
                            if ($y.$md === $billing_ymd) {
                                $billing_day = 7;
                                //preg_match('/SUMMARY:(.+)/', $match, $hit);
                                //$summary = $hit[1];
                                break 2;
                            }
                        }
            ...
            if (in_array($billing_day, $filter)) {
                $json['isHoliday'] = 1;
            }
        }

        if (($json['isHoliday'] ?? null) !== 1 && false !== $this->db->update(
            'receipt',
            [ 'billing_date' => $post['billing_date'] ],
            'userkey = ? AND issue_date = ? AND receipt_number = ? AND templatekey = ?',
            [$this->uid, $issue_date, $receipt_number, $receipt_id]
        )) {
            $this->db->commit();
            ...
            } elseif ($json['isHoliday'] === 1) {
                $json['message'] = Lang::translate('BILLING_DATE_IS_HOLIDAY');
            } else {
                $json['message'] = 'Database Error';
            }
        }
