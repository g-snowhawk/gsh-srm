 ...
 */
class Receipt extends \Gsnowhawk\Srm
{
    /**
    ...
    protected function save(): bool
    {
        ...
        $current_page = $this->session->param(Receipt\Response::RECEIPT_PAGE_KEY);
        ...
        $this->session->param(Receipt\Response::RECEIPT_PAGE_KEY, $current_page);
    ...
    private function newReceiptNumber($issue_date, $draft, $destination = null): ?int
    {
        ...
        $this->session->clear(Receipt\Response::RECEIPT_PAGE_KEY);
    ...
    private function pdfMapping($node_list, $page, $count, &$files)
    {
        ...
        foreach ($node_list as $node_name => $child_node) {
            ...
            $value = [
                ...
                'color' => Pdf::mapAttrToArray((string)$child_node->attributes()->color),
            ...
            if ($child_node->attributes()->ishtml) {
                $value['ishtml'] = Pdf::mapAttrToBoolean(
