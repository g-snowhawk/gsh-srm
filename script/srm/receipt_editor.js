/**
 * This file is part of G.Snowhawk Sales Receipt Management System.
 *
...
function updateBillingDate(event, force) {
    const element = event.target;

    if (event.key === 'Enter') {
        if (element.value.match(/^[0-9]{4}[^0-9][0-9]{1,2}[^0-9][0-9]{1,2}[0-9]?$/)) {
            if (!force && !confirm(element.dataset.confirm)) return;
            const form = element.form;
